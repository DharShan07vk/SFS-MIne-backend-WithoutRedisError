  ...
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      const [psychology] = await tx
        ...
        .values({
          firstName: psychologyDataParsed.data.firstName,
          lastName: psychologyDataParsed.data.lastName,
          city: psychologyDataParsed.data.city,
          state: psychologyDataParsed.data.state,
          mobile: psychologyDataParsed.data.mobile,
          email: psychologyDataParsed.data.email,
          idCardURL: idCardURL,
        })
        .returning();
      res.json({
